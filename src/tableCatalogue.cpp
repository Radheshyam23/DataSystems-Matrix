...
void TableCatalogue::renameTable(string currName, string newName)
{
    logger.log("TableCatalogue::renameTable"); 

    Table* table = new Table(newName);
    table->isMatrix = tableCatalogue.tables[currName]->isMatrix;
    table->sourceFileName = tableCatalogue.tables[currName]->sourceFileName;

    tableCatalogue.deleteTable(currName);
    if (table->load())
    {
        tableCatalogue.insertTable(table);
        cout << "Successfully renamed table" << endl;
    }
    else
    {
        cout << "Failed to rename table" << endl;
    }
